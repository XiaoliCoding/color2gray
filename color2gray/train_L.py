...
def inference(image):
    ...
    weight1 = tf.variable_with_weight_loss(shape=[dim,4096],stddev=0.04,wl=0.004)
    bias1 = tf.Variable(tf.constant(0.1,shape=[4096]))
    local1 = tf.nn.relu(tf.matmul(reshape,weight1) + bias1)
    weight2 = tf.variable_with_weight_loss(shape=[4096,4096],stddev=0.04,wl=0.004)
    bias2 = tf.Variable(tf.constant(0.1,shape=[192]))
    local2 = tf.nn.relu(tf.matmul(local1,weight2) + bias2)
    weight3 = tf.variable_with_weight_loss(shape=[4096,1000],stddev=1/4096.0,wl=0.0)
...
def run_benchmark():
    with tf.Graph().as_default():
        image_size = 224
        image = tf.Variable(tf.random_normal([batch_size,image_size,image_size,3],dtype=tf.float32,stddev=1e-1))
